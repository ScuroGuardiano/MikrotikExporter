...
internal sealed class MikrotikFakeTcpApiConnection : IDisposable
{
    ...
    public async Task<MikrotikResponse> Request(ReadOnlyMemory<byte>[] sentence, CancellationToken cancellationToken = default)
    {
        _client.Rst();
        ...
        await _client.GetStream().CopyToAsync(_buffer, cancellationToken);
        _buffer.Seek(0, SeekOrigin.Begin);
    ...
    private async Task WriteSentence(ReadOnlyMemory<byte>[] words, CancellationToken cancellationToken)
    {
        ...
        await WriteWord(Memory<byte>.Empty, cancellationToken);
    }
    
    private async Task WriteWord(ReadOnlyMemory<byte> word, CancellationToken cancellationToken = default)
    {
        ...
        await WriteBytes(word, cancellationToken);
    }
    ...
    private MikrotikSentence ReadSentence()
    {
        ...
        try
        {
            ...
            var reply = ReadWord(Memory<byte>.Empty);
            ...
            while (true)
            {
                ...
                var (written, overflowed) = ReadWord(buffer.AsMemory(idx, buffer.Length - idx - 1));
    ...
    ///     </ul>
    /// </returns>
    private (int, byte[]?) ReadWord(Memory<byte> target)
    {
        ...
        if (target.Length < len)
        {
            ...
            _buffer.ReadExactly(buffer, 0, len);
            return (0, buffer);
        }
        
        _buffer.ReadExactly(target[..len].Span);
        return (len, null);
    }
    ...
    private uint ReadLength(CancellationToken cancellationToken = default)
    {
        _buffer.ReadExactly(_readLenBuffer, 0, 1);
        ...
        {
            _buffer.ReadExactly(_readLenBuffer, 0, 2);
        ...
        {
            _buffer.ReadExactly(_readLenBuffer, 0, 3);
        ...
        {
            _buffer.ReadExactly(_readLenBuffer, 0, 4);
            c = _readLenBuffer[0];
            c <<= 8;
            c += _readLenBuffer[1];
            c <<= 8;
